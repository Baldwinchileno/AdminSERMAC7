    ...
    public class InventarioForm : Form
    {
        ...
        private void AgregarButton_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in inventarioDataGridView.Rows)
            {
                ...
                string fecha = row.Cells["Fecha"].Value?.ToString() ?? fechaCompra;

                if (!string.IsNullOrEmpty(codigo) && !string.IsNullOrEmpty(producto))
                {
                    try
                    {
                        sqliteService.AddProducto(codigo, producto, unidades, kilos, fechaCompra, fecha);
                    }
                    catch (Exception ex)
                    {
        ...
        {
            using (var crearProductoForm = new CrearProductoForm())
            {
