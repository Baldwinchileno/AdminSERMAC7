                    ...
                    try
                    {
                        ...
                        foreach (DataGridViewRow row in ventasDataGridView.Rows)
                        {
                            ...
                            var command = new SQLiteCommand(@"
                                INSERT INTO Ventas (NumeroGuia, CodigoProducto, Descripcion, Bandejas, 
                                                  KilosNeto, FechaVenta, PagadoConCredito, RUT, Total)
                                VALUES (@numeroGuia, @codigo, @descripcion, @bandejas, 
                                       @kilosNeto, @fechaVenta, @pagadoConCredito, @rut, @total)",
                            ...
                            sqliteService.ActualizarInventario(codigo, bandejas, kilosNeto);
                        }

                        // Si es venta a crédito, actualizar deuda del cliente
                        if (pagadoConCredito == 1)
                        {
                            sqliteService.ActualizarDeudaCliente(rut, totalVenta);
                        }

                        // Incrementar número de guía
                        sqliteService.IncrementarNumeroGuia();
