...
namespace AdminSERMAC.Services
{
    public class SQLiteService
    {
        ...
        private void EnsureTablesExist()
        {
            ...
            EnsureProductosTableExists(); // Nueva línea
        ...
        public void AddProducto(string codigo, string producto, int unidades, double kilos, string fechaCompra, string fechaRegistro)
        {
            using (var connection = new SQLiteConnection(connectionString))
            {
                connection.Open();
                var command = new SQLiteCommand(@"
            INSERT INTO Inventario (Codigo, Producto, Unidades, Kilos, FechaCompra, FechaRegistro)
            VALUES (@codigo, @producto, @unidades, @kilos, @fechaCompra, @fechaRegistro)", connection);
